...
- **Either** a linear beat sequence per act (beat-mode), **or** a graph of unordered nodes per act plus a campaign-level victory node (node-mode). One mode per campaign; the runtime detects which by lorebook content.
- Branch contingencies
- Initial Author's Note state
- Opening hook text (the one file you read before play)

Generated by the campaign generator (see `03_CAMPAIGN_GENERATOR_BRIEF.md`) using a genre pack. Node-mode is the default; pass `--beats-mode` (or `CG_NODE_MODE=0`) to generate the legacy beat-mode campaign instead.
