...
- `setting_statement`: 2-3 sentences of ambient texture, not plot. Sensory and specific — but plain and easy to read, written at a general-audience reading level, not literary fiction. Use short declarative sentences; no comma-spliced run-ons or stacked subordinate clauses; one concrete detail per sentence, not three. This paragraph becomes the voice the GM copies every turn, so it must itself read easily while matching the pitch's mood. NO hard line wraps — write each paragraph as a single long line, separated only by blank lines.
